...
export async function loader({ params, request, context }: DataFunctionArgs) {
  ...
  if (result && result.items) {
    for (const item of result.items) {
      if (item.slug) {
        try {
          const productDetails = await getProductBySlug(item.slug, { request });
          if (productDetails && productDetails.product) {
            item.productDetails = productDetails.product;
          }
        } catch (error) {
          console.error(
            `Error fetching product details for slug ${item.slug}:`,
            error,
          );
